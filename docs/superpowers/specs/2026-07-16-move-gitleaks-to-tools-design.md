...
- `.husky/pre-commit` — use `./tools/bin/gitleaks detect --source . --redact`.
- `README.md` — prerequisites, optional Tooling Git-hooks entry, Quick Install Docker line, and Tooling Git-hooks entry that reference `./tools/bin/gitleaks`.
...
- All tracked references use `./tools/bin/gitleaks`; no tracked references to the obsolete helper location remain.
